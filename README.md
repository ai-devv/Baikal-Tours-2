...
2. Работа с событиями:
    1. Создание/редактирование события (title, короткое описание, полное описание, URL, название, фотографии, выбор главного изображения, создание ALT-тега к главной фотографии, создание дат, проставление локаций, тематик, трансферов, компаньонов, названия организатора, проставление организаторов из справочника пользователей, контактов, соц. сетей, билетов, дополнительных услуг, партнёров события, прикрепления экскурсий, туров и отелей).
    ...
    2. Получение множества событий по конкретному языку. Возможность разбиение результата по страницам, фильтрации по дате начала, дате окончания, локациям, компаньонам, тематикам, начальной цене и конечной цене. Возможность поиска по ключевым словам (название и полное описание события). Для администраторов есть возможность получать события с любым статусом.
    3. Получение одного события по URL (super-festival-veka).
    4. Автоматическое архивирование события по истечению последней даты окончания события.
    5. Оформление броней на событие, отмена броней на событие, оплата броней события через Сбербанка (используется платёжный API).
    6. Письменное оповещение организаторов в некоторый период времени о том, сколько человек зарегистрировано на событие, какие билеты/доп. услуги куплены/забронированы.
    7. Выгрузка подробных данных о купленных/оплаченных билетах и доп. услугах по событию в файл формата «.xlsx». Доступно только для организаторов события.
    8. На странице события показываются два ближайших события, которые подходят по датам и тематикам.
3. Работа с пользователями:
    1. Регистрация пользователя (имя, фамилия, телефон, email, язык). Пароль создаётся случайным образом и отправляется пользователю письмом. В базу данных сохраняется хеш пароля и соль.
    2. Вход пользователя по почте или телефону.
    3. Восстановление пароля по почте. Восстановление возможно максимум 4 раза за 30 дней. При восстановлении новый пароль также высылается на почту.
    4. Получение пользователей с возможностью поиска по имени, фамилии, телефону и почте. Только для администраторов.
    5. Получение/изменение/удаление одного пользователя. Для администратора – любого, для обычного пользователя – только самого себя.
    6. Также, администратор может поменять роль пользователя.
    7. Получение событий, где пользователь является организатором.
    8. Получение всех заявок пользователя на вывод средств.
4. Работа с экскурсиями/турами (т.к., на самом деле, это идентичные сущности):
    1. Создание/редактирование (название, сайт, дата начала, дата окончания, локации, цена, изображение).
    2. Получение множества сущностей с возможностью фильтрации по дате начала, дате окончания, локациям.
    3. Получение одной сущности по id.
    4. Сущности можно «привязать» к событию, тогда на странице события они будут показываться. Важно, если у события нет прикреплённых сущностей, тогда сущности получаются автоматически с фильтрацией по локациям события.
5. Работа с отелями:
    1. Изначально отели были «спарсены» с сайта booking по определённым критериям, это была изначальная база.
    2. Также, из всех локаций всех отелей был составлен справочник локаций отелей (локации букинга).
    3. Создание/редактирование (название, ссылка на booking, фотография, цена, рейтинг, локация букинга).
    4. Получение множества отелей с возможностью разбиения результата по страницам, фильтрации по локациям и локациям букинга. Возможность поиска по названию отеля.
    5. Удаление отеля по id.
    6. Прикрепление отеля к событию. Если у события не будет прикреплённых отелей, то они автоматически получатся с сервера с фильтрацией по тем же локациям, что и у события.
6. Работа с подборками:
    1. Создание/редактирование подборки (URL, title, название, слоган, описание, локации, тематики, изображение). К каждой подборке можно прикрепить события, задав для каждого из них уникальное описание в рамках подборки.
    2. Получение всех подборок с возможностью фильтрации по локациям, тематикам, дате начала и дате окончания.
    3. Получение подборки по URL.
    4. Удаление подборки по id.
7. Работа с избранными событиями:
    1. Создание/редактирование избранных событий по конкретной тематике. Для избранных событий без тематики, соответственно, тематика не нужна. Учитывается порядок элементов.
    2. Получение всех избранных событий с возможностью фильтрации по тематикам. Для избранных событий без тематики реализовано получение всех событий.
8. Работа с заявками на выплату:
    1. Создание заявки (получатель, банк, номер аккаунта, бик, инн, кпп, сумма). Только для организаторов события.
    ...
    2. Принятие заявки. Подразумевается, что человек лично переводит деньги, а через приложение лишь подтверждает этот факт.
    3. Отклонение заявки. Тоже самое, но ещё указывается причина отказа, а также все деньги, зарезервированные для вывода, возвращаются на балансы тех событий, с которых были вычтены.
9. Работа с пересечениями:
    1. Создание/редактирование пересечения (slug – часть URL, описание, интро-текст, заголовок, title). Редактирование по id.
    2. Получение всех пересечений.
    3. Получение одного пересечения по slug.
    4. Удаление пересечения по id.
10. Работа с картой сайта (sitemap.txt):
    1. Есть возможность сгенерировать карту сайта. Автоматически в неё заносятся следующий ссылки:
        1. Собственная ссылка
        2. /events
        3. Ссылки всех активных событий
        4. Ссылки всех подборок
        5. Ссылки всех пересечений
    2. Есть возможность подправить руками, дописать что-то.
    3. Получить карту сайта.
...
17. Почтовый «модуль». Реализована система, при которой письмо делится на три части:
    1. Разметка.
    2. Шаблон текста с некоторыми вставками «{…}» (например, «Привет, {name}!»).
    3. Данные.
...
7. Админка
    ...
    3. Страница редактирования и создания события
    ...
    8. Редактирование и создание экскурсий/туров
    ...
    10. Создание и редактирование отеля
    ...
    12. Создание и редактирование подборки
    ...
    16. Редактирование и создание пересечения
...
# Extra - ER Diagram
![ER Diagram](https://i.ibb.co/NnVVCKM/baikal-tours-er-diagram.png)
